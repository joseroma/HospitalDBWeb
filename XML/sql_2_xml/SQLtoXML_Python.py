...
for row in cur.fetchall():
    ...
    ET.SubElement(especialidad, "nombre_especialidad").text = str(row[14])
    #MEDICO
    medico = ET.SubElement(especialidad, "MEDICO")
    ET.SubElement(medico, "id_medico").text = str(row[15])
    ET.SubElement(medico, "nombre_medico").text = str(row[16])
    #OPERACION
    ...
    ET.SubElement(operacion, "cirujano").text = str(row[19])
    #QUIROFANO
    quirofano = ET.SubElement(operacion, "QUIROFANO")
    ET.SubElement(quirofano, "id_quirofano").text = str(row[20])
    ET.SubElement(quirofano, "planta").text = str(row[21])
    ET.SubElement(quirofano, "puerta").text = str(row[22])
...
xmlfile = open(os.path.join(cd, '../../XML/CLData_Output.xml'),'wb')
xmlfile.write(str.encode('<?xml version="1.0" encoding = "UTF-8"?>\n'))
xmlfile.write(str.encode("<?xml-stylesheet type='html/xsl' href='mydbXSLT.xsl'?>\n"))
